...
reps = 0
timer = None
is_timer_running = False  # New state variable
...
def start_timer():
    ...
    if not is_timer_running:  # Only start if timer isn't already running
...
def count_down(count):
    ...
    if count > 0:
        ...
        is_timer_running = False  # Reset the state when countdown completes
        start_button.config(state="normal")  # Re-enable the start button
        start_timer()
        marks = ""
